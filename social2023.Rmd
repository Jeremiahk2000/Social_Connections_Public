...
```{r gender_pie, echo = FALSE}


# Not using
gender_group <- freshmen %>%
  select(gender) %>%
  count(gender) %>%
...
gender_group %>%
  ...
  geom_text(aes(label = paste0(perc_gender, "%")), size = 3,
            check_overlap = TRUE,
            position = position_stack(vjust = 0.5)) +
  coord_polar("y", start = 0) +
  labs(
    title = "Gender Identity Ratio of All Responses",
  ...
  theme(axis.line=element_blank(),
        ...
        plot.margin = margin(2, 80, 2, 80, "pt"))
...
dorm_group %>%
  ggplot(aes(x = "", y = n, fill = dorm)) +
  geom_bar(stat = "identity", width = 1) +
  geom_text(aes(label = paste0(perc_dorm, "%")), size = 3,
            check_overlap = TRUE,
            position = position_stack(vjust = 0.5)) +
  coord_polar("y", start = 0) +
  labs(
    title = "Dorm Ratio of All Responses",
    fill = "Dorm"
  ) + 
  theme_economist() +
...
```{r create_satisfaction_plot, echo = FALSE. GOOD}
...
```{r satisfaction_scatter_plot, echo = FALSE, GOOD}
...
```{r socially_connected_satisfaction, echo = FALSE, message = FALSE, warning = FALSE}
